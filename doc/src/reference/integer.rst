...
.. function:: round(x\[; n\])
    ...
    Rounds `x` to the nearest number with `n` fractional digits. `n` may be omitted, in which case `x` is rounded to the closest integer. Ties are broken by rounding to the nearest **even** integer. This rounding strategy, commonly known as *Banker's rounding*, serves to avoid a bias (for instance when averaging or summing).
...
.. function:: trunc(x\[; n\])
    ...
    Truncates (rounds toward zero) `x` to the next number with `n` fractional digits. `n` may be omitted, in which case `x` is rounded to integer.
